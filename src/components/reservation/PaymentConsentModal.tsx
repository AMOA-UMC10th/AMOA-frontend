...
export default function PaymentConsentModal({ isOpen, onClose }: PaymentConsentModalProps) {
  if (!isOpen) return null;

  return (
    <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 max-w-[85%] w-full max-h-[70vh] overflow-y-auto mx-6">
        <h2 className="text-lg font-bold mb-3 text-[#171B1C]">취소·환불 규정</h2>
        <p className="text-sm text-[#646F7C] whitespace-pre-line leading-relaxed">
          {REFUND_POLICY_TEXT}
        </p>
        <button onClick={onClose} className="mt-4 w-full bg-[#171B1C] text-white py-2 rounded-lg cursor-pointer">
          닫기
        </button>
      </div>
    </div>
