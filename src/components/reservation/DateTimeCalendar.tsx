...
import { getTodayKey, type TimeSlot } from '../../data/reservationAPI';
...
function isPastTime(selectedDate: string | null, slotTime: string, now: Date) {
  if (!selectedDate) {
    return false;
  }
  ...
  return (
    <div className="px-[16px] pt-6">
          ...
          >
            <span
              className={`text-[11px] font-normal leading-[16.5px] ${
                index === 0 ? 'text-[#F70071]' : 'text-[#646F7C]'
              }`}
            >
              {weekday}
            </span>
      ...
      <div className="grid grid-cols-7">
        {days.map((day, index) => {
          if (day === null) {
            return <div key={`blank-${index}`} className="h-[40px]" />;
          }
        ...
        <div className="mt-6">
          {isLoadingTimes ? (
            <div className="flex justify-center py-40">
              <Spinner size={20} ariaLabel="시간 불러오는 중" />
            </div>
          ) : (
            <div className="grid grid-cols-3 grid-items-center gap-y-[10px] gap-x-[8px]">
              {timeSlots.map((slot) => {
                ...
                const hasPassed = isPastTime(selectedDate, slot.time, now);
